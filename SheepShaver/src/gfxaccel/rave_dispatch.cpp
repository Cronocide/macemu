 ...
 */
// Per-frame RAVE timing (defined in rave_gles_renderer.cpp, "perf_profile").
extern bool   rave_perf;
extern int64  g_rave_dispatch_us;

// RAII timer: accumulates this RaveDispatch call's duration into the per-frame
// total, so NativeRenderEnd can derive guest-geometry time = frame - in-RAVE.
namespace {
struct RaveDispatchTimer {
	uint64 t0;
	bool on;
	RaveDispatchTimer() : t0(0), on(rave_perf) { if (on) t0 = GetTicks_usec(); }
	~RaveDispatchTimer() { if (on) g_rave_dispatch_us += (int64)(GetTicks_usec() - t0); }
};
}
...
{
	RaveDispatchTimer _raveTimer;
	// Read sub-opcode from scratch word
	uint32 method_id = ReadMacInt32(rave_scratch_addr);

	// First-N dispatch trace, gated on the gfx_accel_log runtime flag.
	// os_log() doesn't surface in the H700 console, so we use stderr here
	// when logging is on. Counter is local-static to keep the volume bounded.
