...
// Extracts A(31:24) R(23:16) G(15:8) B(7:0) → writes RGBA8 for GLES GL_RGBA upload.
...
// Same as ARGB32 but top byte (X) ignored, alpha forced to 0xFF. RGBA8 output for GLES.
...
// Same 5-5-5 layout as ARGB16 but top bit ignored, alpha forced to 0xFF. RGBA8 output for GLES.
...
// 8-bit index → CLUT lookup. CLUT stores native RGBA uint32. memcpy preserves byte order.
                      ...
                      uint32_t rowBytes, const uint32_t *clut, uint32_t clutCount)
{
	for (uint32_t y = 0; y < height; y++) {
		...
		for (uint32_t x = 0; x < width; x++) {
			...
			uint32_t rgba = clut[idx];
			uint32_t dstIdx = (y * width + x) * 4;
			memcpy(&dst[dstIdx], &rgba, 4);
		}
	}
}
...
// ConvertRGB16_565: 16bpp, R=15:11(5), G=10:5(6), B=4:0(5). RGBA8 output for GLES.
static void ConvertRGB16_565(uint32 srcAddr, uint8_t *dst, uint32_t width, uint32_t height, uint32_t rowBytes)
{
	for (uint32_t y = 0; y < height; y++) {
		uint32 rowAddr = srcAddr + y * rowBytes;
		for (uint32_t x = 0; x < width; x++) {
			...
			uint8_t b = val & 0x1F;
			uint32_t idx = (y * width + x) * 4;
			dst[idx + 0] = (r << 3) | (r >> 2);   // 5 -> 8
			dst[idx + 1] = (g << 2) | (g >> 4);   // 6 -> 8
			dst[idx + 2] = (b << 3) | (b >> 2);   // 5 -> 8
			dst[idx + 3] = 0xFF;
		}
	}
}
// ConvertRGB24: 24bpp packed, byte order R,G,B in Mac (big-endian) memory.
static void ConvertRGB24(uint32 srcAddr, uint8_t *dst, uint32_t width, uint32_t height, uint32_t rowBytes)
{
	for (uint32_t y = 0; y < height; y++) {
		uint32 rowAddr = srcAddr + y * rowBytes;
		for (uint32_t x = 0; x < width; x++) {
			uint32 px = rowAddr + x * 3;
			uint32_t idx = (y * width + x) * 4;
			dst[idx + 0] = ReadMacInt8(px + 0);   // R
			dst[idx + 1] = ReadMacInt8(px + 1);   // G
			dst[idx + 2] = ReadMacInt8(px + 2);   // B
			dst[idx + 3] = 0xFF;
		}
	}
}
// ConvertAlpha1: 1bpp alpha mask, MSB-first within each byte.  Color is white;
// alpha is 0xFF where the bit is set, 0x00 otherwise.
static void ConvertAlpha1(uint32 srcAddr, uint8_t *dst, uint32_t width, uint32_t height, uint32_t rowBytes)
{
	for (uint32_t y = 0; y < height; y++) {
		uint32 rowAddr = srcAddr + y * rowBytes;
		for (uint32_t x = 0; x < width; x++) {
			uint8_t byte = ReadMacInt8(rowAddr + (x >> 3));
			uint8_t bit = (byte >> (7 - (x & 7))) & 1;
			uint32_t idx = (y * width + x) * 4;
			dst[idx + 0] = 0xFF;
			dst[idx + 1] = 0xFF;
			dst[idx + 2] = 0xFF;
			dst[idx + 3] = bit ? 0xFF : 0x00;
		}
	}
}
                           ...
                           uint32_t rowBytes, const uint32_t *clut, uint32_t clutCount)
{
	for (uint32_t y = 0; y < height; y++) {
		for (uint32_t x = 0; x < width; x++) {
			...
			uint32_t rgba = clut[idx];
			uint32_t dstIdx = (y * width + x) * 4;
			dst[dstIdx + 0] = rgba & 0xFF;         // R from CLUT
			dst[dstIdx + 1] = (rgba >> 8) & 0xFF;  // G from CLUT
			dst[dstIdx + 2] = (rgba >> 16) & 0xFF; // B from CLUT
                          ...
                          uint32_t width, uint32_t height, uint32_t rowBytes)
{
	...
	switch (pixelType) {
		...
		case kQAPixel_RGB16_565:
			ConvertRGB16_565(srcAddr, dst, width, height, rowBytes);
			break;
		case kQAPixel_RGB24:
			ConvertRGB24(srcAddr, dst, width, height, rowBytes);
			break;
		default:
...
// Analyze an expanded RGBA buffer: nonZero = any pixel is non-zero; varied =
// any sampled pixel differs from the first.  A uniform white/black placeholder
// is nonZero but NOT varied, so we can avoid latching pixels_copied onto it and
// keep refreshing until the game writes the real (detailed) texture data.
void RaveAnalyzeExpanded(const uint8_t *expanded, uint32_t totalPixels,
                         bool *nonZero, bool *varied)
{
	bool nz = false, var = false;
	uint32_t stride = (totalPixels > 512) ? totalPixels / 512 : 1;
	uint32_t first = 0;
	bool gotFirst = false;
	for (uint32_t i = 0; i < totalPixels; i += stride) {
		uint32_t pix;
		memcpy(&pix, expanded + i * 4, 4);
		if (pix != 0) nz = true;
		if (!gotFirst) { first = pix; gotFirst = true; }
		else if (pix != first) var = true;
		if (nz && var) break;
	}
	*nonZero = nz;
	*varied = var;
...
/*
 *  RaveExpandTexturePixels - expand a deferred texture, preferring real data.
 *
 *  Direct-format textures defer GL texture creation until draw time and read
 *  the original pixmap then.  Some titles (e.g. Bugdom) have already written
 *  the real texture at TextureNew but reuse/clear that pixmap buffer before the
 *  first draw, so the deferred read sees a uniform placeholder -> all-black or
 *  all-white sprites.  We snapshot the pixmap into cpu_pixel_data at TextureNew;
 *  when the live pixmap looks like a placeholder, fall back to that snapshot.
 */
 void RaveExpandTexturePixels(RaveResourceEntry *entry, uint32_t pixmap,
	uint8_t *dst, bool *outNonZero,
	bool *outVaried, bool *outUsedLivePixmap)
{
	uint32_t w = entry->width;
	uint32_t h = entry->height;
	uint32_t totalPixels = w * h;
	ConvertPixels(entry->pixel_type, pixmap, dst, w, h, entry->row_bytes);
	bool nz = false, var = false;
	RaveAnalyzeExpanded(dst, totalPixels, &nz, &var);
	bool usedLive = true;
	// Live pixmap looks like an empty/uniform placeholder: try the snapshot
	// captured at TextureNew before the guest reused the buffer.
	if (!var && entry->cpu_pixel_mac_addr != 0 && entry->cpu_pixel_mac_addr != pixmap) {
		uint8_t *alt = new uint8_t[totalPixels * 4];
		ConvertPixels(entry->pixel_type, entry->cpu_pixel_mac_addr, alt, w, h, entry->row_bytes);
		bool anz = false, avar = false;
		RaveAnalyzeExpanded(alt, totalPixels, &anz, &avar);
		if (avar || (anz && !nz)) {
			memcpy(dst, alt, totalPixels * 4);
			nz = anz;
			var = avar;
			usedLive = false;
		}
                                         ...
                                         uint32 imagesAddr, RaveResourceEntry *entry)
{
	...
	if (isIndexed) {
		...
		// QD3D often calls TextureNew with a placeholder pixmap and writes the
		// real pixel data later (between TextureNew and first draw).  Reading
		// the pixmap eagerly here races that fill and latches placeholder
		// content (e.g. uniform-white sprite stubs that never refresh because
		// Bugdom never calls TextureDetach).  Deferring matches the PocketShaver
		// Metal backend and lets RaveRealizeDeferredTexture read the pixmap at
		// draw time when QD3D has populated it.
		entry->pixmap_mac_addr = pixmap;
		entry->metal_texture   = nullptr;
		entry->pixels_copied   = false;
		entry->refresh_attempts = 0;

		// For mipmapped textures, cache the TQAImage descriptor array now --
		// the caller's stack may be reused before realize runs.
		...
		RAVE_LOG("TextureNew direct deferred (pixelType=%d) %dx%d mips=%d pixmap=0x%08x",
...
void RaveRealizeDeferredTexture(RaveResourceEntry *entry)
{
	...
	// Detect real (varied) data vs an empty/uniform placeholder.  We create the
	// GL texture regardless (so we don't re-enter realize), but only latch
	// pixels_copied once the data is real -- otherwise the draw path keeps
	// refreshing until the game writes the actual texture (see refresh path).
	// Prefer whichever source (live pixmap or TextureNew snapshot) has real data.
	...
	if (nonZero && (varied || entry->refresh_attempts >= RAVE_TEX_LATCH_ATTEMPTS)) {
		...
		entry->pixels_copied = false;
		if (entry->refresh_attempts < 255) entry->refresh_attempts++;
	}
	...
	// Refresh cpu_pixel_data to match the realized pixel data -- but only when
	// the live pixmap actually held the real data.  Otherwise we'd overwrite the
	// good TextureNew snapshot (our fallback source) with a placeholder.
	if (entry->cpu_pixel_data && entry->cpu_pixel_mac_addr && usedLivePixmap) {
		Host2Mac_memcpy(entry->cpu_pixel_mac_addr, Mac2HostAddr(pixmap), entry->cpu_pixel_data_size);
	}

	RAVE_LOG("TextureRealize: pixelType=%d %dx%d mips=%d pixmap=0x%08x -> metal=%p nonZero=%d varied=%d latched=%d",
	       pixelType, w, h, mipLevels, pixmap, entry->metal_texture,
	       (int)nonZero, (int)varied, (int)entry->pixels_copied);
}
                                      ...
                                      int32_t transparentIndex, RaveResourceEntry *entry)
{
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		uint8_t b = rgb & 0xFF;
		uint8_t a = 0xFF;
		// Store as RGBA in native byte order for GLES GL_RGBA upload
		clut[i] = r | (g << 8) | (b << 16) | (a << 24);
	}
...
int32_t NativeEngineTextureDetach(uint32_t textureAddr)
{
	...
	if (handle == 0) return kQANoErr;
	RaveResourceEntry *entry = RaveResourceGet(handle);
	if (!entry) return kQANoErr;

	// RAVE spec: TextureDetach is the definitive "pixel data is final" signal --
	// the engine must (re)copy the pixmap now, before the app reuses the buffer.
	// This is the reliable completion point (no per-frame polling): if the
	// texture was realized early during a draw, it may have latched onto
	// placeholder/empty data (white or black), so force a fresh upload of the
	// final contents here regardless of pixels_copied.
	if (entry->pixmap_mac_addr != 0) {
		if (!entry->metal_texture) {
			RaveRealizeDeferredTexture(entry);
		} else {
			entry->pixels_copied = false;	// clear the latch so refresh re-uploads
...
int32_t NativeEngineAccessTextureEnd(uint32_t textureAddr, uint32_t dirtyRectAddr)
{
	...
	// Re-upload (or create) the GL texture
	if (entry->metal_texture) {
		RaveUploadMipLevel(entry->metal_texture, 0, w, h, expanded, w * 4);
		if (entry->mip_levels > 1)
			RaveGenerateMipmaps(entry->metal_texture);
	} else {
		entry->metal_texture = RaveCreateMetalTexture(w, h, entry->mip_levels, expanded, w * 4);
	}
	entry->pixels_copied = true;

	delete[] expanded;
	RAVE_LOG("AccessTextureEnd: tex=0x%08x re-uploaded %dx%d metal=%p", textureAddr, w, h, entry->metal_texture);
	return kQANoErr;
}
