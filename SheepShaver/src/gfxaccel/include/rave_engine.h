...
struct RaveResourceEntry {
	...
	uint8_t          refresh_attempts; // number of times refresh found empty data (caps re-read overhead)
...
extern void RaveAnalyzeExpanded(const uint8_t *expanded, uint32_t totalPixels,
                                bool *nonZero, bool *varied);
// Expand a deferred (direct-format) texture's pixels into RGBA8 `dst`, choosing
// the source with real, varied data.  Prefers the live pixmap, but falls back
// to the snapshot captured in cpu_pixel_data at TextureNew when the live pixmap
// looks like an empty/uniform placeholder (the guest may have already reused
// that buffer).  Reports the chosen source's analysis plus whether the live
// pixmap was the source used (so callers know whether to refresh cpu_pixel_data).
extern void RaveExpandTexturePixels(RaveResourceEntry *entry, uint32_t pixmap,
	uint8_t *dst, bool *outNonZero,
	bool *outVaried, bool *outUsedLivePixmap);
// Frames a deferred texture may keep refreshing while its data still looks like
// a uniform placeholder, before latching anyway (bounds overhead for solid
// textures).
#define RAVE_TEX_LATCH_ATTEMPTS 60
